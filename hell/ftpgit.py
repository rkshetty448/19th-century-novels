...
class GitHubFTPHandler(FTPHandler):
    ...
    def ftp_RNFR(self, filename):
        ...
        try:
            ...
            parts = full_path.strip("/").split("/", 1)
            if len(parts) < 2:
                self.respond("550 Cannot move or rename a repository root. Use GitHub's web interface.")
                return
    ...
    def _move_directory(self, old_repo, old_path, new_repo, new_path):
        """Recursively move a directory across repositories."""
        contents = old_repo.get_contents(old_path)
        if not isinstance(contents, list):
            contents = [contents]
        ...
        for item in contents:
            ...
            commit_message = f"FTP move: {old_item_path} to {new_item_path}"

            if item.type == "file":
                data = base64.b64decode(item.content)
                new_repo.create_file(new_item_path, commit_message, data)
                old_repo.delete_file(old_item_path, f"FTP move: delete {old_item_path}", item.sha)
                print(f"[DEBUG] Moved file {old_item_path} to {new_item_path}")
            elif item.type == "dir":
                self._move_directory(old_repo, old_item_path, new_repo, new_item_path)
    ...
    def ftp_RNTO(self, filename):
        ...
        try:
            ...
            parts = full_path.strip("/").split("/", 1)
            if len(parts) < 2:
                self.respond("550 Invalid destination: must include a repository and path (e.g., /repo_name/path).")
                return
            ...
            old_repo = self.github.get_user().get_repo(old_repo_name)
            new_repo = self.github.get_user().get_repo(new_repo_name)

            # Check if destination already exists
            try:
                new_repo.get_contents(new_file_path)
                self.respond("550 Destination file or directory already exists.")
                print(f"[DEBUG] RNTO failed: {new_file_path} already exists in {new_repo_name}")
                return
            except GithubException as e:
                if e.status != 404:
                    raise
            ...
            if isinstance(file_content, list):  # Directory
                self._move_directory(old_repo, old_file_path, new_repo, new_file_path)
            else:  # File
                data = base64.b64decode(file_content.content)
                commit_message = f"FTP move: {old_file_path} to {new_file_path}"
                new_repo.create_file(new_file_path, commit_message, data)
                old_repo.delete_file(old_file_path, f"FTP move: delete {old_file_path}", file_content.sha)
                print(f"[DEBUG] Moved file {old_file_path} to {new_file_path}")

            self.respond("250 Move successful.")
            print(f"[DEBUG] Move completed from {old_repo_name}/{old_file_path} to {new_repo_name}/{new_file_path}")
            self.rename_from = None
            
        except GithubException as e:
            if e.status == 404 and "Not Found" in str(e):
                self.respond(f"550 Destination repository not found: {new_repo_name}")
                print(f"[DEBUG] RNTO error: Repository {new_repo_name} does not exist")
            else:
                self.respond(f"550 Failed to move: {str(e)}")
...
def main():
    ...
    print("Use ' performs cript.py addtoken' to add or update your GitHub tokens")
